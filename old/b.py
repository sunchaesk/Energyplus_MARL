...
class EnergyPlusEnv(gym.Env):
    ...
    def _retrieve_actuator_values(self):
    ...
    def step(self, action_l):
        ...
        if self.energyplus_runner.simulation_complete:
            ...
            timeout = 2
            try:
                self.act_queue.put(sat_spt_value_l, timeout=timeout)
        ...
        actuator_values = tuple(self._retrieve_actuator_values())

        obs_vec = np.array(list(obs.values()))
        return obs_vec, reward_energy, done, False, {'actuator_values': actuator_values}
...
if __name__ == "__main__":
    ...
    for episode in range(1):
        ...
        while not done:
            ...
            print('ACTION:', action)
            print('ACTUATOR VALUES:', info['actuator_values'])
            print('reward:', reward)
